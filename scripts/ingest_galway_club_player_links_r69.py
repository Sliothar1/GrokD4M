"""Round 69: AF-first PIHC 2026 path (Portumna beat AF score unpublished; PQF Kilconieron draw); Kinvara Intermediate County Final team-sheet thin fills (Aodhan O'Hanrahan + Tadhg Heavey dual-cited with CT 2025 IHC final; Liam Martyn full name); Portumna 3-16 Kinvara 0-14 PIHC R3; ME IHC 2025 joint-captains; orphans HOLD; club-less ~79 HOLD. Data in data/r69/pack-data.json."""
...
DATA_PATH = ROOT / "data" / "r69" / "pack-data.json"
...
def main():
    ...
    for need in [
        ...
        "club:skehana", "club:mountbellew-moylough", "club:leitrim-galway-historic", "club:ballinasloe", "club:fohenagh-historic", "club:ahascragh-historic", "club:rahoon", "club:fr-joe-walshs", "club:newcastle-galway", "club:eyrecourt-historic", "club:kilbeacanty",
    ]:
    ...
    for pm in PLAYER_META:
        pid = pm["id"]
        if pid not in by_row or by_row[pid].get("type") != "player":
            stats["player_meta_missing"] += 1
            continue
        for col, val in pm["cols"].items():
            if col == "note" and by_row[pid].get("note"):
                oldn = by_row[pid]["note"]
                if val not in oldn:
                    set_or_add(seed, pair_index, by_row, pid, "note", (oldn + " " + val).strip(), stats)
                continue
            set_or_add(seed, pair_index, by_row, pid, col, val, stats)
        stats["player_meta_touched"] += 1
    ...
    for pid, hnote in hold_notes.items():
        ...
        if hnote not in note and "HOLD r69" not in note:
    ...
    for clear_pid, clear_club in [
        ("player:vin-baston-army", "club:army-galway"),
        ("player:joe-young-army", "club:army-galway"),
    ]:
    ...
    pack = {
        ...
        "generated_at": now,
        "sources": [
            "https://www.kinvaragaa.ie/articles/134183",
            "https://www.connachttribune.ie/sport/hurling/meelick-eyrecourt-finish-in-a-blaze-of-glory-to-end-famine-5594766",
            "https://www.galwaybayfm.ie/sports/meelick-eyrecourt-1-16-kinvara-0-16-intermediate-hurling-final-report-reaction-206455",
            "https://www.balls.ie/gaa/joe-canning-portumna-galway-gaa-662762",
            "https://www.galwaybayfm.ie/sports/draws-announced-for-county-senior-and-intermediate-hurling-preliminary-quarter-finals-229399",
            "https://hoganstand.com/County/Galway/Article/Index/341673",
        ...
        "still_orphan_priority": [
            c for c in [
                "club:claregalway", "club:eyrecourt-historic",
                "club:kilrickle", "club:oranmore-historic", "club:skehana", "club:st-colemans",
                "club:fr-joe-walshs", "club:cois-fharraige",
            ] if c in after["orphan_ids"]
        ],
        "orphan_note": "Remaining HOLD orphans: Eyrecourt/Kilrickle/St Coleman's/Fr Joe Walsh's/Cois Fharraige (no first+last named XV cites). r69: Claregalway orphan broken via Tom Ruane 1910–1916 parish hurling captain; AF Fohenagh 1959/1960 SHC captains stamped; Billy Duffy → Meelick-Eyrecourt; club-less ~79 stay HOLD.",
    }
    ...
    log = {
        "url": "Kinvara Intermediate team-sheet fills (Aodhan/Tadhg dual-cite; Liam Martyn); Portumna 3-16 Kinvara 0-14 PIHC R3; AF PQF Kilconieron; ME IHC joint-captains; orphans HOLD",
        "date": "2026-09-11",
        "title": "Galway club-player pack r69",
        "publisher": "Kinvara GAA / Connacht Tribune / Galway Bay FM / Balls.ie / HoganStand / HurlingWiki",
        "processed_at": now,
    ...
    if fohenagh_new or fohenagh_linked:
        q = {
            "batch": "fohenagh-club-links-r69",
            "ruled_at": now,
            "clear_links": fohenagh_linked,
            "unverified_new": fohenagh_new,
            "note": "AF/Fohenagh r69: PIHC 2026 path notes (Portumna R2 win score unpublished; PQF Kilconieron); Kinvara team-sheet fills opposing AF R1; ME IHC joint-captains; club-less HOLD; remaining HOLD orphans unnamed.",
        }
        existing = []
