...
3. The same associative-array algebra as hurling: sparse `(row, col, val)` with `getrow` / `getcol` / `search`. Marine row pattern: `station:{id} / {year}-W{week}@{location}` (**537** triples).
